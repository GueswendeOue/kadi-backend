...
- Cycle : `CLIENT` → `ARTICLE` → `ARTICLE_DECISION` → `ARTICLE` ou `OPTIONS` → `DOCUMENT_ESTIMATE`.
...
Variables documentées : `KADI_INVOICE_FLOW_ENABLED`, `KADI_INVOICE_FLOW_ID`, `KADI_INVOICE_FLOW_MODE`, `KADI_INVOICE_FLOW_ENDPOINT_URL`, `KADI_INVOICE_MAX_ITEMS`, `KADI_FLOW_PRIVATE_KEY`, `KADI_FLOW_PRIVATE_KEY_PASSPHRASE`.
