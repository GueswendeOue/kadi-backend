...
// fix/kadi-v1-receipt-discharge — forward-only migration adding
// RECEIPT_DETAILS to kadi_v1_conversation_sessions_expected_flow_key_check.
// This never touches any previously applied migration; it only asserts the
// new pair of files.

const ROOT = path.resolve(__dirname, "..");
const SUPABASE_FILE = path.join(ROOT, "supabase", "migrations", "20260805040000_add_kadi_v1_receipt_details_flow_key.sql");
const MIGRATIONS_FILE = path.join(ROOT, "migrations", "20260805_add_kadi_v1_receipt_details_flow_key.sql");
...
test("the already-applied migrations were not touched", () => {
  const appliedSupabase = path.join(ROOT, "supabase", "migrations", "20260805030000_add_kadi_v1_invoice_type_flow_key.sql");
  const appliedMigrations = path.join(ROOT, "migrations", "20260805_add_kadi_v1_invoice_type_flow_key.sql");
  const supabaseSql = fs.readFileSync(appliedSupabase, "utf8");
  const migrationsSql = fs.readFileSync(appliedMigrations, "utf8");
  assert.doesNotMatch(supabaseSql, /RECEIPT_DETAILS/);
  assert.doesNotMatch(migrationsSql, /RECEIPT_DETAILS/);
...
test("the new constraint contains RECEIPT_DETAILS exactly once, right after INVOICE_TYPE, plus every previous value", () => {
  ...
  assert.equal(listedValues.filter((value) => value === "RECEIPT_DETAILS").length, 1, "RECEIPT_DETAILS must appear exactly once");
  assert.equal(listedValues.indexOf("RECEIPT_DETAILS"), listedValues.indexOf("INVOICE_TYPE") + 1, "RECEIPT_DETAILS must sit right after INVOICE_TYPE");
  for (const value of PREVIOUS_17_VALUES) assert.ok(EXPECTED_VALUES.includes(value), `${value} must be preserved`);
  assert.equal(listedValues.length, PREVIOUS_17_VALUES.length + 1, "exactly one new value must be added");
  assert.equal(listedValues.length, 18, "the constraint must list exactly 18 values");
});
...
test("no existing Flow ID env var name is reused: KADI_V1_FLOW_RECEIPT_DETAILS_ID is new", () => {
  const { FLOW_ENV_KEYS } = require("../kadiV1RuntimeConfig");
  const values = Object.values(FLOW_ENV_KEYS);
  assert.equal(values.filter((v) => v === "KADI_V1_FLOW_RECEIPT_DETAILS_ID").length, 1);
  assert.equal(new Set(values).size, values.length, "every flow env var name must be unique");
});
