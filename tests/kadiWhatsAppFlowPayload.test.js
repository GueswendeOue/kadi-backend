...
test("payload forbids screen, draft and business data in the initial message", () => {
  for (const forbidden of [
    { screen: "CLIENT" },
    { data: {} },
    { data: { draft_id: "draft-1" } },
  ]) {
    assert.throws(() => buildDraftInvoiceFlowMessage({ ...validArgs, ...forbidden }), /FLOW_INITIAL_DATA_FORBIDDEN/);
  }
  const serialized = JSON.stringify(buildDraftInvoiceFlowMessage(validArgs));
  assert.doesNotMatch(serialized, /flow_action_payload|draft_id|client_name|items_summary/);
});
